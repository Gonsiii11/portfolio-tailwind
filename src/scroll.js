...
const animations = {
    ...
    init(container = document) {
        ...
        this.initNavbarScroll(container);
    ...
    initHeaderProfile() {
        const profile = document.querySelector('#navbar .group');
        if (!profile) return;

        // Establecemos estado inicial
        gsap.set(profile, { x: -20, opacity: 0 });

        // Animamos hacia su posición final
        gsap.to(profile, {
            x: 0,
            opacity: 1,
            duration: 1,
    ...
    initNavbarScroll() {
        const nav = document.querySelector('#navbar div'); // Seleccionamos el contenedor interno
